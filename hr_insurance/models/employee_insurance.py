...
class EmployeeInsurance(models.Model):
    ...
    policy_id = fields.Many2one(
        "insurance.policy", string="Policy", required=True, help="Policy"
    )
    ...
    def get_policy_period(self):
        ...
        if self.policy_coverage == "yearly":
            self.date_to = str(
                datetime.now() + relativedelta.relativedelta(months=+12)
            )[:10]

    employee_percentage = fields.Integer(string="Employee percentage")

    result = fields.Integer(string="result", compute="_compute_result")

    @api.depends("employee_percentage", "result", "amount")
    def _compute_result(self):
        self.result = self.amount * (self.employee_percentage / 100)
    ...
    def weight_employee(self):
        self.state = "confirm"
        self.employee_id.deduced_amount_per_month = (
            self.employee_id.deduced_amount_per_month + self.result
        )
